        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    aHtml.push('<p class="review_img_tmb">'+sImg+'</p>');
...
$(document).ready(function(){
    if($(".photoreview-outputNumber").length > 0) {
        // 상품포토후기 갯수 설정.
        var count = "10";
        setTimeout(function() {
		    $(".photoreview-outputNumber option").eq(0).val(count);
        	$(".photoreview-outputNumber").change();
		}, 10);
    };
});
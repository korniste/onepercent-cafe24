$(document).ready(function(){
    ...
    var config;
    var sort;
    var moreBtn = $(".skin_grid3 .prv_table").next(".skin_grid3 .more");
    var readyFlag = true;
    ...
    function init()
    {
        if (reviewer && reviewer.data.length > 0) {
            ...
            $('.skin_grid3 .prv_table td img[src=""]').hide();
            initData();
            setEvt();
        } else {
			$('.skin_grid3').remove();
        ...
        $(".skin_grid3 .prv_table tbody").show();
    }
   ...
   function textOverCut(txt, len, lastTxt) {
    ...
    function setEvt()
    {
        $(document).delegate(".skin_grid3 .rv_btn_close", 'click', function(e){
            e.preventDefault();
            var tr = $(this).parent().parent();
            if (tr.hasClass('selected')) {
                $(".skin_grid3 .prv_table").find("tr").removeClass("selected");
                $(".skin_grid3 .reviewDetail").hide().remove();        
            } else {
                $(".skin_grid3 .prv_table").find("tr").removeClass("selected");
                $(".skin_grid3 .reviewDetail").hide().remove();
                ...
                $(".skin_grid3 .reviewDetail").show();
            }
        });
		
        $(document).delegate(".skin_grid3 .preview_detail", 'click', function(e){
            e.preventDefault();
            var tr = $(this).parent().parent();
            if (tr.hasClass('selected')) {
                $(".skin_grid3 .prv_table").find("tr").removeClass("selected");
        ...
        // 작업 카운트 개수
		var count = 3;
        $(".skin_grid3 .photoreview-outputNumber").change(function(e){
            var val = $(this).val();
			// 작업 count
        ...
        $(".skin_grid3 .photoreview-viewMode").change(function(e){
            var val = $(this).val();
        ...
        $(document).delegate(".skin_grid3 .photoreview .photoreview-paging a", "click", function(e){
            e.preventDefault();
    ...
    function page_refresh(list)
    {
        ...
        if (data && data.length > 0) {
            $(".skin_grid3 .prv_table tbody").hide().html('');
            for(var i=0; i<data.length; i++) {
                var cell = makeCell(data[i]);
                if (cell) {
                    $(".skin_grid3 .prv_table tbody").append(cell);
                }
            }
            
            if (typeof list.pagination == "string") {
                $(".skin_grid3 .photoreview .photoreview-paging").html(list.pagination);
            }
    ...
    function showDetail(tr, no)
    {
        if (rowData[no]) {
            ...
            if (row) {
				...
				// 작업 이미지슬라이더 부분
                setTimeout(function() {
                    $(".skin_grid3 .reviewArea .imgBox .item:first").css("opacity","1");
                    $(".skin_grid3 .reviewArea .imgBox .item:first").addClass("active");
                    if($(".skin_grid3 .reviewArea .imgBox .item.active").next().length == 0) $(".skin_grid3 .reviewArea .imgBox .next").hide();
                    if($(".skin_grid3 .reviewArea .imgBox .item.active").prev().length == 0) $(".skin_grid3 .reviewArea .imgBox .prev").hide();
                }, 1);
                setTimeout(function() {
                    $(".skin_grid3 .reviewArea .imgBox .next").click(function (e) {
                         $dom = $(".skin_grid3 .reviewArea .imgBox .item.active").next();
                         $(".skin_grid3 .reviewArea .imgBox .item").removeClass("active");
                         $dom.addClass("active");
                         if($(".skin_grid3 .reviewArea .imgBox .item.active").next().length == 0) $(".skin_grid3 .reviewArea .imgBox .next").hide();
                         if($(".skin_grid3 .reviewArea .imgBox .item.active").prev().length == 0) {
                         	 $(".skin_grid3 .reviewArea .imgBox .prev").hide();   
                         }else {
                         	 $(".skin_grid3 .reviewArea .imgBox .prev").show();    
                         }
                    });
                    $(".skin_grid3 .reviewArea .imgBox .prev").click(function (e) {
                         $dom = $(".skin_grid3 .reviewArea .imgBox .item.active").prev();
                         $(".skin_grid3 .reviewArea .imgBox .item").removeClass("active");
                         $dom.addClass("active");
                         if($(".skin_grid3 .reviewArea .imgBox .item.active").prev().length == 0) $(".skin_grid3 .reviewArea .imgBox .prev").hide();
                         if($(".skin_grid3 .reviewArea .imgBox .item.active").next().length == 0) {
                         	 $(".skin_grid3 .reviewArea .imgBox .next").hide();   
                         }else {
                         	 $(".skin_grid3 .reviewArea .imgBox .next").show();    
                         }                        
                    })
                }, 10);                 
            }
    ...
    {
        if (gdetail === true) {
            $('.skin_grid3 .photoreview .is_detail').remove();
        }

        if (guse_point !== true) {
            $('.skin_grid3 .photoreview .is_rating').remove();
        }
    }
	...
	$(".skin_grid3 .xans-photoreview-display .prv_table .subject .photoreview.list").each(function(k,v) {
		var writer = $(this).find(".skin_grid3 .writer").html();
		var dat = $(this).find(".skin_grid3 .date").html();
		$(this).find(".skin_grid3 .writer").remove();
		$(this).find(".skin_grid3 .date").remove();
		$(this).html($(this).text());

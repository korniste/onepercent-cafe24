$(document).ready(function(){
    ...
    cellSize = (isNaN(cellSize) || cellSize <= 0) ? 181 : cellSize;
    ...
    function init()
    {
        ...
        if (reviewer && reviewer.data.length > 0) {
            ...
            $(".xans-photoreview-display").addClass('disnone');
        }
...
//        });
    ...
    function makeCell(c)
    {
        ...
        tmpl.push('<dl>');
        tmpl.push('<dt><a href="#none" rel="'+c.no+'" style="background-image:url('+img+');"></a></dt>');
        tmpl.push('<dd>');
        tmpl.push('<p class="sbj elp">'+c.rsub+'</p>');
        ...
        tmpl.push('<p class="post">'+c.rwriter+'<span>'+c.rdate+'</span>'+'</p>');
        tmpl.push('</dd>');
        tmpl.push('</dl>');
        return tmpl.join('');
    }
    ...
    function showDetail(no)
    {
        if (rowData[no]) {
            ...
            if (row) {
                ...
                $(".xans-photoreview-display .detail .post .rwriter").html(row.rwriter);
                $(".xans-photoreview-display .detail .post .rdate").html(row.rdate);
                $(".xans-photoreview-display .detail .image").html('');
                if (imgs.length > 0) {
                    var img_html = '';
                    for (var i=0; i<imgs.length; i++) {
                        img_html += '<img src="'+imgs[i]+'" /><br/>';
                    }
